...
fn too_short() {
    unsafe {
        assert_eq!(guarded_transmute_vec_permissive::<u16>(vec![]), Ok(vec![]));
        assert_eq!(guarded_transmute_vec_permissive::<u16>(vec![0x00]), Ok(vec![]));
    }
}
...
fn just_enough() {
    unsafe {
        assert_eq!(guarded_transmute_vec_permissive::<u16>(vec![0x00, 0x01].le_to_native::<u16>()), Ok(vec![0x0100u16]));
